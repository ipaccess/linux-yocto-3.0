 ...
 */
#define IPAC_KEYSTREAM_BUFFER_SIZE  (0x80000)

/* We assume that there will never be more than this number of PDUs in a
 * single request.  This value is used when copying the counts that are
 * used in the IV.  In practice there are normally about 17 80 bytes PDUs
 * in a single request.  That could potentially double if the PDU size was
 * 40 bytes.  In uplink there may be a lot more in 10ms, particularly if
 * PDUs are buffered to allow in-sequence delivery with a window of up to
 * 2047.
 */
#define IPAC_MAX_NUM_PDUS            1536
...
enum IpacStateTag
{
    ...
    IPAC_REQUEST_SKIP       /* Request that's subsequently been cancelled */
};
...
struct IpacRequestTag {
    ...
    unsigned                   *key_p;        /* Pointer to the key that was used for this request */
