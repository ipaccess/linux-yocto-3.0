...
#define CARDNAME "spi_pc302"

#ifdef CONFIG_ARCH_PICOXCELL
...
static int __init pc302spi_probe(struct platform_device *dev)
{
	...
	/* Enable the clock to the SPI block and read its rate */
	c->clk = clk_get(&dev->dev, NULL);
	if (!c->clk || IS_ERR(c->clk))
	{
	...
	c->min_speed_hz = (c->baseclk + PC302_SPI_MAX_CLK_DIVIDER - 1)/ PC302_SPI_MAX_CLK_DIVIDER;
	c->max_speed_hz = c->baseclk / PC302_SPI_MIN_CLK_DIVIDER;
...
#ifdef CONFIG_ARCH_PICOXCELL
	if (!devm_request_mem_region(&dev->dev, res->start, res->end - res->start + 1, CARDNAME))
	{
		goto exit_busy;
	}
...
#ifdef CONFIG_ARCH_PICOXCELL
  //Code to replace syscfg_update in the older BSP
        u32 syscfg = axi2cfg_readl(AXI2CFG_SYSCFG_REG_OFFSET);  
        u32 syscfg_mask = AXI2CFG_SYSCFG_SSI_EBI_SEL_MASK;
        syscfg &= ~syscfg_mask;
        syscfg |= (system_config & syscfg_mask);
        axi2cfg_writel(syscfg, AXI2CFG_SYSCFG_REG_OFFSET);
#else
	printk("Updating SYS_CONFIG mask=0x%08X, value=0x%08X\n", AXI2CFG_SYSCFG_SSI_EBI_SEL_MASK, system_config);
#endif
...
#ifdef CONFIG_ARCH_PICOXCELL
	ret = devm_request_irq(&dev->dev, irq, pc302spi_interrupt, 0, CARDNAME, c);
	if (ret)
	{
		goto exit;
	}
#else
	printk("Register irq %d for %s with pc302spi_interrupt\n", irq, CARDNAME);
#endif
	...
	master->mode_bits      = SPI_3WIRE | SPI_MODE_3 | SPI_MODE_1;
