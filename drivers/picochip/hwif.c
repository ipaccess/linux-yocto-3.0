...
hwif_queue_transfer( struct dma_channel *channel )
{
    ...
    if ( size > 0 )
    {
        ...
        sg_init_one( &sgl, dma_fifo_get_virt_writeptr( channel->fifo ),
                 channel->xferSize );
        sgl.dma_address = dma_fifo_get_writeptr( channel->fifo );

        /* Report how much data we are going to accept */
        ret = pa->ops->register_write( pa, channel->count, channel->xferSize );
        if ( ret )
            PRINTD( COMPONENT_HWIF, DBG_ERROR,
                "Failure to request bytes to receive Sent=%u "
                "Transfered=%u Total=%u bytes",
                channel->bytesSent, channel->xferSize, channel->totalBytes );

        ret = pa->ops->dma_from_device( pa, channel->chan, &sgl, channel->xferSize,
