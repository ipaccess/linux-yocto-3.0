/**
 * drivers/net/ksz_ptp.h - Micrel PTP common header
 *
...
#define SCALED_NANOSEC_SHIFT		16
#define SCALED_NANOSEC_MULT		(1 << SCALED_NANOSEC_SHIFT)
...
struct ptp_msg_info {
	...
	struct ptp_msg msg;
	struct ptp_utime rx;
	struct ptp_utime tx[MAX_PTP_PORT];
	int state[MAX_PTP_PORT];
	s64 residence[MAX_PTP_PORT];
};

struct ptp_packet {
	struct ptp_msg_info sync;
	struct ptp_msg_info req;
	struct ptp_msg_info follow_up;
	int cnt;
	int tx_cnt;
	struct ptp_packet *prev;
	struct ptp_packet *next;
};
...
struct p2p_time {
	...
	struct ptp_utime t3;
	struct ptp_utime t4;
	struct ptp_utime t1_org;
	struct ptp_utime t2_org;
	struct ptp_utime t3_org;
	struct ptp_utime t4_org;
	int seqid;
	s64 sync_pdelay_resp;
	s64 follow_up;
	s64 delay_resp;
	int set;
	int peer_delay;
...
struct e2e_time {
	...
	u16 port;
	u16 seqid;
	struct p2p_time delay_resp[MAX_DELAY_REQ_CNT];
};
...
#define NUM_OF_TIMESTAMP		(1 << 4)

struct ptp_rx_queue {
	struct ptp_rx_ts rx[NUM_OF_TIMESTAMP];
	int head;
	int read;
	int tail;
};
...
#define PTP_SIM_2_STEP			(1 << 28)
#define PTP_SYNT			(1 << 29)
#define PTP_MON				(1 << 30)

/* Software overrides. */

#define CHECK_TS_NUM			(1 << 5)
#define CHECK_TS_MAX			(CHECK_TS_NUM - 1)
...
struct ptp_info {
	...
	struct work_struct proc_ptp_irq;
	struct {
		u8 buf[MAX_TSM_UDP_LEN];
		int len;
	} udp[MAX_TSM_UDP_CNT];
	...
	struct ptp_time time_diff;
	u32 sec_hi;

	struct ptp_utime sync_in_time;
	struct ptp_utime sync_time;
	struct ptp_utime sync_time_adj;
	struct ptp_utime in_time;
	struct ptp_utime in_time_org;
	int in_port;

	struct ptp_ts check_ts[CHECK_TS_NUM];
	struct ptp_ts saved_ts[CHECK_TS_NUM];
	int cur_ts;
	int last_ts;
	...
	struct ptp_output outputs[MAX_TRIG_UNIT];
	int udp_head;
	int udp_tail;
	int dev_major;
	...
	struct work_struct set_clk;
	struct work_struct sync_clk;
	struct work_struct synt_clk;
#ifdef PTP_MONITOR
#ifdef PTP_PROCESS
	struct work_struct exit_state;
#endif
	struct work_struct ts_rx;
	struct ptp_rx_queue rx;
	struct ptp_clock_identity clock_id[CLOCK_ENTRIES];

	/* monitor path delay. */
	struct e2e_time e2e;
	struct p2p_time p2p[MAX_PTP_PORT];
	int master_port;

	struct ptp_utime pdelay_t2[MAX_PTP_PORT];
	int path_delay;
	int path_offset;
	struct ptp_id tx_id;
	struct ptp_irig_info irig;
	int delay_send;

#ifdef PTP_2_STEP
	struct work_struct proc_2step;
	struct ptp_packet pkt_head;
	int pkt_paused;
	int sim_2_step;
	struct sk_buff *sync_skb;
	struct sk_buff *dreq_skb;
#endif

	int sync_init_cnt;
	unsigned long last_correct_jiffies;
	int software_running;
	unsigned long get_time_jiffies;
#endif
	unsigned long last_dbg_jiffies;
};
