/*****************************************************************************
 * BSP Version: 4.0.6, RevisionID: 4bd13fa, Date: 20110823 09:53:27
 *****************************************************************************/
 ...
 *
 * Copyright (c) 2008-2011 Picochip Ltd.
...
#include "pc3xx_emac.h"
#include "../picoxcell_core.h"
#ifdef CONFIG_PC3XX_MICREL_VLAN_SWITCH
#include "pc3xx_i2c_phy.h"
#endif /* CONFIG_PC3XX_MICREL_VLAN_SWITCH */
...
/*!
 * Strings array used for ethtool stats reporting
 */
static char pc302emac_stats_gstrings[][ETH_GSTRING_LEN] = {
    "tx_octets",
    "tx_frames",
    "tx_broadcast_frames",
    "tx_multicast_frames",
...
#define ETHTOOL_STATS_LEN ARRAY_SIZE(pc302emac_stats_gstrings)
...
#ifndef CONFIG_PC3XX_MICREL_VLAN_SWITCH
/*!
...
static void
pc302emac_get_wol(struct net_device *dev,
                  struct ethtool_wolinfo *wol);

static int
pc302emac_set_wol(struct net_device *dev,
                  struct ethtool_wolinfo *wol);
static int
pc302emac_get_sset_count(struct net_device *dev,
                         int sset);

static void
pc302emac_get_strings(struct net_device *dev,
                      u32 stringset,
                      u8 *buf);

static void
pc302emac_get_ethtool_stats(struct net_device *dev,
...
#else /* CONFIG_PC3XX_MICREL_VLAN_SWITCH */
...
pc302emac_tx(struct net_device *dev)
{
    ...
    if (status & (EMAC_TRANSMIT_UNDERRUN | EMAC_TRANSMIT_RETRY_LIMIT_EXCEEDED))
    {
        int i;

	printk(KERN_ERR "%s: Tx %s, resetting buffers\n",
			 dev->name, status & EMAC_TRANSMIT_UNDERRUN ?
 			 "underrun" : "retry limit exceeded");

        /* Disable the transmitter before clearing up the buffers
         * Note: This also resets the tx buffer base address register
         *       to point to the start of the tx descriptor chain.
         */
        control = emac_ioread32(dev, EMAC_NETWORK_CTRL_REG_OFFSET);
		   ...
		   unsigned int last_frag)
{
    ...
    if (priv->hw_timestamp_flag == HWTSTAMP_FILTER_ALL)
    {
        ...
        shhwtstamps->hwtstamp = hw_timestamp;
    }
                    ...
                    void *dev_id)
{
	...
	if (unlikely(!netif_running(dev)))
        ...
        if (status & (EMAC_ENABLE_TX_COMPLETE |
                      EMAC_ENABLE_TX_BUFF_UNDERRUN |
                      EMAC_ENABLE_RETRY_LIMIT_EXCEEDED))
        {
        ...
        if (status & (EMAC_ENABLE_EXTERNAL_INTERRUPT))
            printk(KERN_INFO "%s: woken by magic packet\n", dev->name);
...
pc302emac_init_hw(struct net_device *dev)
{
    ...
    /* Make sure that we do not throw away the Frame checksum for
     * received packets as this is required for correct operation
     * of packet timestamping.
     */
    config &= ~(EMAC_FCS_REMOVE);
    ...
    /*
     * PC3X3 uses the wol output of the EMAC looped back to the external
     * interrupt pin to provide Wake-on-LAN support.
     */
    if (picoxcell_is_pc3x3())
        interrupt_enable |= EMAC_ENABLE_EXTERNAL_INTERRUPT;
...
net_device_stats *pc302emac_get_stats(struct net_device *dev)
{
    ...
    stats->tx_errors = (hwstat->tx_late_collisions +
                        hwstat->tx_excessive_collisions +
                        hwstat->tx_underrun +
                        hwstat->tx_carrier_sense_errors);

    stats->multicast = hwstat->rx_multicast_frames;
...
/* Ethtool functions and data structures */
static int
pc302emac_get_settings(struct net_device *dev,
                       struct ethtool_cmd *cmd)
{
	              ...
	              struct ethtool_drvinfo *info)
{
    ...
    strcpy(info->bus_info, dev_name(&phydev->dev));
}
...
pc302emac_get_wol(struct net_device *dev,
                  struct ethtool_wolinfo *wol)
{
    struct pc302_emac *emac = netdev_priv(dev);

    if (picoxcell_is_pc3x3() && device_can_wakeup(&emac->pdev->dev))
    {
        u32 value = emac_ioread32(dev, EMAC_WOL_REG_OFFSET);
        wol->supported = WAKE_MAGIC;
        wol->wolopts = value & EMAC_WOL_MAGIC_PACKET_ENABLE ? WAKE_MAGIC : 0;
    }
    else
    {
        wol->supported = 0;
                  ...
                  struct ethtool_wolinfo *wol)
{
    ...
    spin_lock_bh(&emac->lock);

    if (wol->wolopts & WAKE_MAGIC)
    {
        device_set_wakeup_enable(&emac->pdev->dev, true);
        emac_iowrite32(dev, EMAC_WOL_MAGIC_PACKET_ENABLE, EMAC_WOL_REG_OFFSET);
        irq_set_irq_wake(dev->irq, 1);
    }
    else
    {
        device_set_wakeup_enable(&emac->pdev->dev, false);
        irq_set_irq_wake(dev->irq, 0);
    }

    spin_unlock_bh(&emac->lock);
...
pc302emac_get_strings(struct net_device *dev,
                      ...
                      u8 *buf)
{
    memcpy(buf, pc302emac_stats_gstrings,
           (ETHTOOL_STATS_LEN * ETH_GSTRING_LEN));
}
                            ...
                            uint64_t *data)
{
    ...
    for (i = 0; i < ETHTOOL_STATS_LEN; i++)
    {
        if (!strncmp (pc302emac_stats_gstrings[i], "tx_octets", 9) ||
            !strncmp (pc302emac_stats_gstrings[i], "rx_octets", 9))
        {
            /* Special case because we need to concatinate the
             * two tx_octet counts or the two rx_octet counts from
             * the hardware counters, into a single count value
             * for ethtool stats reporting
             */
            data[i] = (uint64_t)p[j++];
            data[i] |= ((uint64_t)p[j++]) << 32;
        }
        else
        {
            data[i] = (uint64_t)p[j++];
...
ethtool_ops pc302emac_ethtool_ops =
{
    ...
    .get_wol            = pc302emac_get_wol,
    .set_wol            = pc302emac_set_wol,
    .get_sset_count     = pc302emac_get_sset_count,
    .get_strings        = pc302emac_get_strings,
    .get_ethtool_stats  = pc302emac_get_ethtool_stats,
};
...
static struct net_device_ops pc302_netdev_ops = {
    .ndo_open = pc302emac_open,
    .ndo_stop = pc302emac_close,
    .ndo_start_xmit = pc302emac_start_xmit,
    .ndo_get_stats = pc302emac_get_stats,
    .ndo_set_multicast_list = pc302emac_set_rx_mode,
    .ndo_do_ioctl = pc302emac_ioctl,
    .ndo_vlan_rx_register = emac_vlan_rx_register,
    .ndo_vlan_rx_kill_vid = emac_vlan_rx_kill_vid,
};
...
emac_drv_probe(struct platform_device *pdev)
{
    ...
    dev->netdev_ops = &pc302_netdev_ops;
    ...
    printk(KERN_INFO "%s: Ethernet driver loaded\n",
           CARDNAME);
    ...
    /*
     * The EMAC on PC3X3 supports Wake-On-LAN (WOL).
     */
    if (picoxcell_is_pc3x3())
...
#ifdef CONFIG_PM
static int
emac_suspend(struct platform_device *pdev,
             pm_message_t state)
{
    ...
    /* Disable all interrupts apart from magic packet. */
    emac_iowrite32(dev, ~EMAC_ENABLE_EXTERNAL_INTERRUPT,
		   EMAC_INT_DISABLE_REG_OFFSET);
    emac_iowrite32(dev, EMAC_ENABLE_EXTERNAL_INTERRUPT,
		   EMAC_INT_ENABLE_REG_OFFSET);

    return 0;
}

static int
emac_resume(struct platform_device *pdev)
{
    struct net_device *dev = platform_get_drvdata(pdev);
    struct pc302_emac *priv = netdev_priv(dev);
