/**
 * drivers/net/ksz_sw_sysfs.c - Micrel switch common sysfs code
 *
 * Copyright (c) 2011-2012 Micrel, Inc.
	...
	char *buf, unsigned long offset)
{
	...
	get_private_data(d, &proc_sem, &sw, &hwlock, &port);
	...
	sw_acquire(sw, hwlock);
	len = sysfs_sw_read_hw(sw, proc_num, len, buf);
	sw_release(sw, hwlock);
	...
	char *buf, unsigned long offset)
{
	...
	get_private_data_3(d, &proc_sem, &sw, &hwlock);
	if (down_interruptible(proc_sem))
		return -ERESTARTSYS;
	...
	char *buf, unsigned long offset)
{
	...
	get_private_data_2(d, &proc_sem, &sw);
	if (down_interruptible(proc_sem))
		return -ERESTARTSYS;
...
static struct ksz_dev_attr ksz_sw_dev_attrs[
	MAC_ATTRS_SIZE * STATIC_MAC_TABLE_ENTRIES +
	VLAN_ATTRS_SIZE * VLAN_TABLE_ENTRIES +
	SW_ATTRS_SIZE * TOTAL_PORT_NUM];
static struct ksz_dev_attr *ksz_sw_dev_attrs_ptr = ksz_sw_dev_attrs;

static void exit_sw_sysfs(struct ksz_sw_sysfs *info, struct device *dev)
{
	int i;
...
static int init_sw_sysfs(struct ksz_sw_sysfs *info, struct device *dev)
{
	int err;
	int i;
