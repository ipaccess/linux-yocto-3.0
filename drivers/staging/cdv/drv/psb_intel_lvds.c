...
static void psb_intel_lvds_set_actual_backlight(struct drm_device *dev, int level)
{
	u32 blc_pwm_ctl;
...
static void psb_intel_lvds_enable_backlight(struct drm_device *dev)
{
	struct drm_psb_private *dev_priv =
		(struct drm_psb_private *) dev->dev_private;
	
	if (dev_priv->mode_dev.backlight_duty_cycle == 0)
		dev_priv->mode_dev.backlight_duty_cycle =
			psb_intel_lvds_get_max_backlight(dev);
	
	dev_priv->mode_dev.backlight_enabled = 1;
	psb_intel_lvds_set_actual_backlight(dev, dev_priv->mode_dev.backlight_duty_cycle);	
}

static void psb_intel_lvds_disable_backlight(struct drm_device *dev)
{
	struct drm_psb_private *dev_priv =
		(struct drm_psb_private *) dev->dev_private;
	
	dev_priv->mode_dev.backlight_enabled = 0;
	psb_intel_lvds_set_actual_backlight(dev, 0);	
}
...
void psb_intel_lvds_set_backlight(struct drm_device *dev, int level)
{
	...
	dev_priv->mode_dev.backlight_duty_cycle = level;
	if (dev_priv->mode_dev.backlight_enabled)
		psb_intel_lvds_set_actual_backlight(dev, level);
}
				 ...
				 struct psb_intel_output *output, bool on)
{
	...
	if (on) {
		...
		psb_intel_lvds_enable_backlight(dev);
	} else {
		psb_intel_lvds_disable_backlight(dev);
...
static int psb_intel_panel_update_status(struct backlight_device *bd)
{
	...
	psb_intel_lvds_set_backlight(dev, bd->props.brightness);
	return 0;
}
...
static int psb_intel_panel_setup_backlight(struct drm_device *dev, struct drm_connector *connector)
{
	...
	dev_priv->backlight->props.brightness = psb_intel_lvds_get_backlight(dev);
	return 0;
}

static void psb_intel_panel_destroy_backlight(struct drm_device *dev)
{
	struct drm_psb_private *dev_priv = dev->dev_private;
		     ...
		     struct psb_intel_mode_device *mode_dev)
{
	...
	psb_intel_get_pwm_ctl(dev);
	mode_dev->backlight_enabled = !!(mode_dev->backlight_duty_cycle);
	DRM_DEBUG_KMS("backlight duty cycle %x\n", mode_dev->backlight_duty_cycle);
out:
	drm_sysfs_connector_add(connector);

	psb_intel_panel_setup_backlight(dev, connector);
